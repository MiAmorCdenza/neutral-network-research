...
- 定位：架构 L0/L1 逐条落地。覆盖条目映射：静息电位与阈值→L0.F1；膜电位时间积分（LIF）→L0.F8；不应期→L0.F2；树突区隔化→L0.F3；除法归一化（Carandini & Heeger）→L0.F4；能量预算→L0.F5；通道化分工→L0.F6；内在可塑性（稳态双通道）→L0.F7；电突触→L1.F1；AMPA/NMDA/mGluR 三通路→L1.F2–F4；释放概率 Pr→L1.F5/F6；STP→L1.F7；传导延迟→L1.F8；三通路汇总→L1.F9；相位复用→L9（架构 L1 注明）。
- 时钟域：全部 T0（每推理步递推）；状态必须注册为 buffer 并提供 reset()。

## L0 物理硬件层（节点级）

### L0.F1 `node_activation(v, theta, straight_through=False) -> z`
- 生物：全或无——膜电位过阈值发放单个动作电位；输出为二进制尖峰 z ∈ {0,1}，**不携带幅度**（幅度信息交给下游突触滤波：L1.F2–F4 的时间滤波天然以尖峰序列为输入）。
- 算法：`z = 1{v > θ}`；`straight_through=True`（仅基线路径）时梯度按指示函数直通。
- 特性：[纯函数]；输入为膜电位 v（L0.F8 递推产物），不再是瞬时输入 h；spike_mode 配置：binary（全或无，默认完整档）/ amplitude（z = relu(v−θ)，率近似降采样档，须显式标注）；稀疏性由 θ 校准（D4-5：活跃率 10–20%；校准机制 = L0.F7 内在可塑性）；θ 为细参数（个体化）、阈值形式为粗参数（全网络统一，通道化）。
...
### L0.F8 `membrane_integrate(v, i_syn, tau_m, theta, v_reset, dt) -> (v', spike)`
- 生物：膜电位时间积分（LIF）——连续小波刺激在**空间**（L0.F3 段内巧合检测）与**时间**（本函数，τ_m ≈ 20 ms 等效）上积累；过阈值触发一次动作电位后复位（全或无）。静息基线 = 0（偏置 b 并入 v 初始值，DEV.F1 生成）。
- 算法：`v_tmp = v + (dt/τ_m)·(−v + i_syn)`；`spike = L0.F1(v_tmp, θ)`（全或无判定）；`v' = spike·v_reset + (1−spike)·v_tmp`。
- 特性：[递推状态 v][数值安全]；spike ∈ {0,1}；链序：F3（空间）→ F8（时间）→ F1（发放）→ F2（不应期）；i_syn = L0.F3 段和（树突空间积分的产物）。
